...
#ifdef sch1
void test_problem (population *pop, int size, int ind) {
    pop->obj[0*size + ind] = pow(pop->xreal[0*size + ind],2.0f);
    pop->obj[1*size + ind] = pow((pop->xreal[0*size + ind]-2.0f),2.0f);
    return;
}
#endif
...
void test_problem (float *xreal, float *xbin, int **gene, float *obj, float *constr)
{
    ...
    for (i=0; i<nreal; i++)
    {
        s1 += pow((xreal[i]-(1.0/sqrt((float)nreal))),2.0);
        s2 += pow((xreal[i]+(1.0/sqrt((float)nreal))),2.0);
    }
    obj[0] = 1.0 - exp(-s1);
    obj[1] = 1.0 - exp(-s2);
    return;
...
#ifdef kur
void test_problem (float *xreal, float *obj, float *constr)
{
    int i;
    float res1, res2;
    res1 = -0.2f*sqrt((xreal[0]*xreal[0]) + (xreal[1]*xreal[1]));
    res2 = -0.2f*sqrt((xreal[1]*xreal[1]) + (xreal[2]*xreal[2]));
    obj[0] = -10.0f*( exp(res1) + exp(res2));
    obj[1] = 0.0f;
    for (i=0; i<3; i++)
    {
        obj[1] += pow(fabs(xreal[i]),0.8f) + 5.0f*sin(pow(xreal[i],3.0f));
    }
    return;
}
#endif
